...
export default function ResetPasswordPage() {
  ...
  const [otp, setOtp] = useState(""); // optional, in case backend sends OTP
  ...
  // If token present in query, we skip email step and allow password set immediately
  const handleProceed = async () => {
    ...
    setLoading(true);
    try {
      // Request a reset token/OTP be sent to the email. Some APIs use /auth/reset-request or /auth/forgot.
      ...
      setInfo("We sent instructions to your email. Check inbox and follow the link or paste the token below.");
      setStep("password");

      // If server returned token directly (dev mode), pre-fill it and proceed
      if (data?.token) {
        // navigate to same page with token in query so flow continues
  ...
  const handleResetPassword = async () => {
    ...
    setLoading(true);
    try {
      // Prefer token from query; if not available, expect user to paste OTP into `otp`
      const token = tokenFromQuery || otp;
      if (!token) {
        setError("No reset token found. Please check your email for the reset link or token.");
        setLoading(false);
        return;
      }
      ...
      const res = await fetch(`${API_BASE}/auth/reset`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token, password: newPassword, email: email || undefined }),
      });
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="flex items-center justify-center py-16 px-6">
        <div className="w-full max-w-md bg-white p-8 rounded-lg shadow-md border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
          ...
          {step === "password" && (
            <div className="space-y-6">
              {/* If there is no token in query, show OTP input so user can paste it */}
              {!tokenFromQuery && (
                <div className="flex flex-col gap-2">
                  <label className="font-medium text-gray-700 dark:text-gray-300">
